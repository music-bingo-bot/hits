...
def create_app(bot):
    ...
    async def upload_track(
        ...
        # автоимя "Музыкальное бинго - 01.mp3"
        # считаем следующий номер = текущих + 1
        items = await get_all_tracks()
        seq = len(items) + 1
        ...
        await create_track(title or f"Хит #{seq:02d}", audio_path or "", hint_path)
        return RedirectResponse("/admin_web", status_code=302)
    ...
    async def edit_track_post(
        ...
        if audio and audio.filename:
            os.makedirs("uploads/audio", exist_ok=True)
            # сохранить под тем же именем что и было либо под новым читаемым
            ext = os.path.splitext(audio.filename)[1] or ".mp3"
            audio_path = os.path.join("uploads","audio", audio.filename if audio.filename.endswith(".mp3") else f"{title or 'hit'}{ext}")
            async with aiofiles.open(audio_path, "wb") as f:
                while chunk := await audio.read(1024*64):
                    await f.write(chunk)
        ...
        await update_track(track_id, title, audio_path, hint_path)
        return RedirectResponse("/admin_web", status_code=302)
    ...
    async def broadcasts_send(request: Request, bid:int):
        ...
        # достаём текст/заголовок из таблицы broadcasts (для простоты — через список)
        ...
        # готовим альбом (не больше 10 элементов по правилам TG)
        album = []
