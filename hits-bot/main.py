...
async def cb_start(c: CallbackQuery):
    ...
    need = max(MIN_TRACKS, 1)
    if len(tracks) < need:
        await safe_send_text(
            c.message.answer,
            f"⚠️ В плейлисте {len(tracks)} трек(ов). Нужно ≥ {need}. Загрузите через /admin_web."
        )
        await c.answer()
        return
...
async def send_current_track(chat_id: int):
    ...
    try:
        # локальный путь (volume) или file_id/url
...
async def cb_restart(c: CallbackQuery):
    ...
    if len(tracks) < need:
        await safe_send_text(
            c.message.answer,
            f"⚠️ В плейлисте {len(tracks)} трек(ов). Нужно ≥ {need}."
        )
        await c.answer()
        return
...
async def keepalive():
    import aiohttp
    host = next(
        (os.getenv(k, "").strip() for k in ("PUBLIC_URL", "RAILWAY_PUBLIC_DOMAIN", "REPLIT_WEB_URL") if os.getenv(k)),
        ""
    )
    if not host:
        return
